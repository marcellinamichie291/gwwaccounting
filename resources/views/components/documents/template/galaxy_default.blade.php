            ...
            <div class="text">
                @stack('company_logo_start')
                    @if (! $hideCompanyLogo)
                        ...
                        @else
                            <img class="d-logo w-image" src="{{ $logo }}" alt="{{ data_get($document, 'company.name') }}"/>
                        @endif
                    @endif
        ...
        <div class="col-42">
            <div class="text right-column">
                ...
                @if (! $hideCompanyDetails)
                    @if (! $hideCompanyName)
                        <p>{{ data_get($document, 'company.name') }}</p>
                    @endif

                    @if (! $hideCompanyAddress)
                        <p>
                            {!! nl2br(data_get($document, 'company.address')) !!}
                    ...
                    @if (! $hideCompanyTaxNumber)
                        ...
                        @if (data_get($document, 'company.tax_number'))
                            <p>
                                {{ trans('general.tax_number') }}: {{ data_get($document,'company.tax_number') }}
                            </p>
                        @endif
                    ...
                    @if (! $hideCompanyPhone)
                        @if (data_get($document, 'company.phone'))
                            <p>
                                {{ data_get($document, 'company.phone') }}
                            </p>
                        @endif
                    @endif

                    @if (! $hideCompanyEmail)
                        <p class="small-text">{{ data_get($document,'company.email') }}</p>
                    @endif
                @endif
